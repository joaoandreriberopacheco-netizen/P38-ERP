 ...
 * Quebra de página por **linha de 3 cards** (não pela secção inteira) para evitar vazios.
 */
...
export function montarHtmlSecaoFolhaAnaloga({ folha, spx, escapeHtml, formatCurrency, labelData }) {
  ...
  const COLS = 3;
  const linhasGridHtml = [];
  for (let i = 0; i < folha.linhas.length; i += COLS) {
    const slice = folha.linhas.slice(i, i + COLS);
    const cells = slice.map(artigoDeLinha).join('');
    // Células vazias para manter 3 colunas na última linha incompleta
    const fillers = Array.from({ length: COLS - slice.length }, () => '<div aria-hidden="true"></div>').join('');
    linhasGridHtml.push(
      `<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px;break-inside:avoid;page-break-inside:avoid">${cells}${fillers}</div>`,
    );
  }

  const secaoHtml = `<section style="margin-top:10px;border-radius:10px;overflow:visible;background:#eef2f7;break-inside:auto;page-break-inside:auto">
    <div style="padding:8px 6px;background:#e2e8f0;border-bottom:1px solid #cbd5e1">
