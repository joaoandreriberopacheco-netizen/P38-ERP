...
export default function AutoLostSales({ open, onClose }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[85dvh] overflow-y-auto font-din-1451 rounded-2xl border-[#e8ecef]/80 p-6 sm:p-8">
        <div className="mb-6">
          <p className={AUTO_EYEBROW}>Ajuda</p>
          <h2 className={`${AUTO_DISPLAY} text-2xl sm:text-3xl mt-1`}>Não encontrou?</h2>
          <p className={`${AUTO_SUBHEADING} mt-2`}>Conte o que você estava procurando.</p>
        </div>
        <div className="space-y-4">
          <div className="relative">
            ...
            {filteredSugestoes.length > 0 && (
              <div className={`absolute z-10 w-full mt-1 ${AUTO_SURFACE_CLASS} overflow-hidden`}>
                {filteredSugestoes.map((s, i) => (
                  <button
                    key={i}
                    type="button"
                    className="w-full text-left p-3 hover:bg-[#f8fafb] text-sm text-[#404040]"
                    onClick={() => setMsg(s)}
                  >
                    {s}
                  </button>
          ...
          <div className="flex gap-3">
            <Input
              ...
              className={`w-24 text-center ${AUTO_FIELD_CLASS}`}
              aria-label="Quantidade"
            />
            <Button
              onClick={handleSubmit}
              disabled={!msg.trim()}
              className={`flex-1 ${AUTO_PRIMARY_BTN}`}
            >
              Enviar
            </Button>
          </div>
          <Button variant="ghost" onClick={onClose} className={`${AUTO_GHOST_BTN} border-0 shadow-none text-[#6b6b6b]`}>
            Cancelar
          </Button>
        </div>
      </DialogContent>
