...
/** Recalcula totais a partir de PedidoCompraItem — espelho JSON `itens[]` deixou de ser gravado. */
const recomporPedido = async (base44: any, pedidoId: string) => {
  ...
  const valorItens = round6(
    ordenadas.reduce((acc: number, it: any) => acc + asNumber(it?.total, 0), 0),
  );
