-- =============================================================================
-- 097 — Corrigir órfãos: saldo só dentro do desmembrado (não pedido inteiro)
-- =============================================================================
    ...
    round(
      (
        case
          ...
          else greatest(coalesce(mr.quantidade_embarcada, 0) - coalesce(mr.quantidade_recebida, 0), 0)
        end
...
comment on view public.pedido_compra_orfaos_v is
  'Órfãos por linha: saldo dentro do desmembrado (splits − recebido) ou embarcado − recebido '
  'quando o pedido já foi todo desmembrado; inclui Necessidade pós-recepção. '
  'Não conta parte do pedido que ainda não entrou em nenhum embarque.';
...
comment on view public.pedido_compra_orfaos_resumo_v is
  'Resumo de órfãos por pedido (migration 097).';
