-- =============================================================================
-- 107 — Saldo a embarcar: fator do pedido por produto_id quando PCI link ausente
-- =============================================================================
-- Bug (ex.: FKJ-2GF): embarque_item sem pedido_compra_item_id e sem dados.fator;
-- migration 106 caía em fator=1 → 30 CX contados como 30 M² (não 75 M²).
-- Fix: resolver fator via pedido_compra_item (produto_id + pedido) antes do fallback 1.
-- =============================================================================
...
create or replace function public.p38_pci_fator_para_embarque_item(
  p_pedido_compra_id text,
  p_produto_id text,
  p_pedido_compra_item_id text default null,
...
stable
as $$
  select public.p38_embarque_item_fator(
    p_ei_dados,
    coalesce(
      (
        select nullif(pci.fator_aplicado, 0)
        from public.pedido_compra_item pci
        where pci.pedido_compra_id = p_pedido_compra_id
          and pci.produto_id = p_produto_id
        order by
          case when p_pedido_compra_item_id is not null and pci.id = p_pedido_compra_item_id then 0 else 1 end,
          pci.ordem
        limit 1
      ),
      1::numeric
...
comment on function public.p38_pci_fator_para_embarque_item(text, text, text, jsonb) is
  'Fator da linha de embarque: dados JSONB → pedido_compra_item (id ou produto_id) → 1.';
...
grant execute on function public.p38_embarque_item_qty_base(numeric, jsonb, text, numeric)
  to authenticated, anon, service_role;
grant execute on function public.p38_pci_fator_para_embarque_item(text, text, text, jsonb)
  to authenticated, anon, service_role;
  ...
  select
    ...
    public.p38_embarque_item_qty_base(
      ...
      public.p38_pci_fator_para_embarque_item(
        ec.pedido_compra_id, ei.produto_id, ei.pedido_compra_item_id, ei.dados
      )
...
comment on view public.pedido_compra_saldo_a_embarcar_v is
  'Saldo a embarcar. Base agregada com fator embarque (107: produto_id + pedido); vitrine na leitura.';
