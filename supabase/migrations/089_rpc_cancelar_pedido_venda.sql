...
-- Cancela pedido de venda mantendo histórico: estorna estoque, cancela financeiro,
-- marca logística como Cancelado. Paridade com Exclusão de Documentos sem DELETE.
...
begin
  ...
  -- Financeiro: cancelar lançamentos vinculados e estornar saldo de pagos
  for v_lanc in
    select l.*
  ...
  -- Estoque: entrada de cancelamento para cada saída de venda ainda não estornada
  for v_mov in
    select m.*
  ...
  -- Vale troca: restaurar saldo se usado no pagamento
