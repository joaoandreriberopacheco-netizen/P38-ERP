-- 034_fix_processar_venda_caixa_rascunho_columns.sql
-- Após 031_promote_remaining_from_dados, rascunho_pedido_venda guarda itens/totais
-- em colunas promovidas; a RPC lia só dados jsonb → pedido_venda vazio (total 0).
...
-- Reparo: vendas já processadas com total/itens vazios mas rascunho com dados nas colunas.
do $$
declare
  v_rec record;
  v_item jsonb;
...
begin
  for v_rec in
    select
      ...
      r.observacoes,
      r.senha_atendimento,
      pv.dados as pv_dados
    from public.pedido_venda pv
    join public.rascunho_pedido_venda r
      on coalesce(r.pedido_venda_final_id, r.dados->>'pedido_venda_final_id') = pv.id
    where coalesce(pv.total, 0) = 0
      and coalesce(r.valor_total, 0) > 0
      and coalesce(jsonb_array_length(coalesce(pv.itens, '[]'::jsonb)), 0) = 0
      and coalesce(jsonb_array_length(coalesce(r.itens, '[]'::jsonb)), 0) > 0
  loop
    v_pedido_dados := coalesce(v_rec.pv_dados, '{}'::jsonb) || jsonb_build_object(
      'itens', coalesce(v_rec.itens, '[]'::jsonb),
      'subtotal', coalesce(v_rec.subtotal::text, '0'),
      'valor_desconto', coalesce(v_rec.valor_desconto::text, '0'),
      'valor_frete', coalesce(v_rec.valor_frete::text, '0'),
      'valor_total', coalesce(v_rec.valor_total::text, '0'),
      'cliente_id', v_rec.cliente_id,
      'cliente_nome', coalesce(v_rec.cliente_nome, ''),
      'vendedor_id', v_rec.vendedor_id,
      'vendedor_nome', coalesce(v_rec.vendedor_nome, ''),
      'tabela_preco_id', coalesce(v_rec.tabela_preco_id, ''),
      'tipo', coalesce(v_rec.tipo, 'Venda'),
      'metodo_entrega', coalesce(v_rec.metodo_entrega, ''),
      'observacoes', coalesce(v_rec.observacoes, ''),
      'senha_atendimento', coalesce(v_rec.senha_atendimento, '')
    );

    update public.pedido_venda
      set total = coalesce(v_rec.valor_total, 0),
          itens = coalesce(v_rec.itens, '[]'::jsonb),
          cliente_id = nullif(v_rec.cliente_id, ''),
          cliente_nome = nullif(v_rec.cliente_nome, ''),
          subtotal = v_rec.subtotal,
          valor_desconto = v_rec.valor_desconto,
          valor_frete = v_rec.valor_frete,
          tipo = v_rec.tipo,
          metodo_entrega = v_rec.metodo_entrega,
          observacoes = v_rec.observacoes,
          vendedor_id = v_rec.vendedor_id,
          vendedor_nome = v_rec.vendedor_nome,
          tabela_preco_id = v_rec.tabela_preco_id,
          senha_atendimento = v_rec.senha_atendimento,
          dados = v_pedido_dados
    where id = v_rec.pedido_id;

    update public.rascunho_pedido_venda
      set pedido_venda_final_id = v_rec.pedido_id,
          data_conversao = coalesce(data_conversao, now())
    where id = v_rec.rascunho_id;

    if not exists (
      select 1 from public.movimentacao_estoque where referencia_id = v_rec.pedido_id
    ) then
      for v_item in select * from jsonb_array_elements(coalesce(v_rec.itens, '[]'::jsonb))
      loop
        v_qtd_base := coalesce(
          nullif(v_item->>'quantidade_base', '')::numeric,
          nullif(v_item->>'quantidade', '')::numeric,
        ...
        insert into public.movimentacao_estoque (
          ...
          coalesce(v_rec.vendedor_nome, 'Operador'),
          coalesce(nullif(v_item->>'custo_unitario_momento', '')::numeric, 0),
          jsonb_build_object(
            'produto_id', v_item->>'produto_id',
            'produto_nome', v_item->>'produto_nome',
            ...
            'custo_unitario', coalesce(nullif(v_item->>'custo_unitario_momento', '')::numeric, 0),
            'referencia_tipo', 'PedidoVenda',
            'referencia_id', v_rec.pedido_id,
            'referencia_numero', v_rec.numero,
            'cliente_nome', coalesce(v_rec.cliente_nome, ''),
            'usuario_responsavel', coalesce(v_rec.vendedor_nome, 'Operador')
          )
        );
      end loop;
    end if;
