...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .pedido-table col.col-foto { width: 52px; }
    .pedido-table col.col-modelo { width: 28%; }
    .pedido-table col.col-qty { width: 68px; }
    .pedido-table col.col-m2,
    .pedido-table col.col-m2u { width: 82px; }
    .pedido-table col.col-cx { width: 68px; }
    .pedido-table col.col-peso { width: 92px; }
    .pedido-table col.col-emb { width: 128px; }
    .pedido-table col.col-preco { width: 84px; }
    .pedido-table col.col-sub { width: 98px; }
    html[data-skin="formigres"] .pedido-table { min-width: 860px; }
    ...
    .pedido-table tbody .pedido-col-qty { font-weight: 700; color: var(--text-strong); }
    .pedido-table .col-subtotal {
      font-weight: 700;
      color: var(--accent-bright);
      white-space: nowrap;
    }
    .pedido-table .pedido-col-peso { font-weight: 600; white-space: nowrap; }
    .pedido-table .pedido-col-preco { font-size: .8rem; }
    .pedido-table .pedido-col-foto img { display: block; border-radius: 6px; }
    ...
    .pedido-card-qty { min-width: 52px; text-align: center; padding: 0 2px; align-self: start; }
    ...
    .pedido-card-spec th + th,
    .pedido-card-spec td + td {
      padding-left: 8px;
      border-left: 1px solid var(--border-subtle);
    }
    .pedido-card-spec-head th {
      padding-bottom: 6px;
      font-size: .62rem;
      font-weight: 500;
      color: var(--muted);
    ...
    .pedido-card-spec-body td.is-peso {
      font-weight: 700;
      color: var(--accent-bright);
    }
    .pedido-card-spec-body td.pedido-card-spec-preco .preco-orig {
      display: block;
      font-size: .58rem;
      font-weight: 400;
    ...
    .pedido-card-spec-body td.pedido-card-spec-preco .preco-desc { font-weight: 600; }
    .pedido-card-spec-body td.pedido-card-spec-preco:not(.has-desc) { font-size: .76rem; }
    .pedido-card-spec-body td.pedido-card-spec-palete {
      font-size: .68rem;
      font-weight: 500;
      line-height: 1.3;
    }
    ...
    @media (max-width: 720px) {
      ...
      .pedido-card-spec-head th { font-size: .58rem; }
      .pedido-card-spec-body td { font-size: .72rem; }
      ...
      .model-table thead th:nth-child(4) { display: none; }
      ...
      .col-qty, .model-table thead th:nth-child(5) {
        position: sticky;
        right: 0;
        background: var(--surface);
        z-index: 1;
      ...
      .model-table thead th:nth-child(5) { background: var(--surface-2); z-index: 2; }
      .col-qty .qty-input {
        width: 48px;
        min-height: 40px;
        font-size: .92rem;
    ...
    @media (max-width: 480px) {
      ...
      .col-qty, .model-table thead th:nth-child(5) { width: 54px; min-width: 54px; }
...
<body class="is-loading">
  ...
  <script>
    ...
    function renderPedidoSpecTable(cells) {
      const head = cells.map((c) => '<th scope="col">' + esc(c.label) + '</th>').join('');
      const body = cells.map((c) => {
        const cls = [
          c.peso ? 'is-peso' : '',
          c.preco ? 'pedido-card-spec-preco' + (descontoPct ? ' has-desc' : '') : '',
          c.palete ? 'pedido-card-spec-palete' : '',
        ].filter(Boolean).join(' ');
        const val = c.html != null ? c.html : esc(c.text ?? '—');
        return '<td' + (cls ? ' class="' + cls + '"' : '') + '>' + val + '</td>';
      }).join('');
      return '<table class="pedido-card-spec" role="presentation">' +
        '<tr class="pedido-card-spec-head">' + head + '</tr>' +
        '<tr class="pedido-card-spec-body">' + body + '</tr>' +
      '</table>';
    }
    function renderPedidoCardRow1({ thumb, titulo, item, qty, m2tot, sub }) {
      ...
      return '<div class="pedido-card-row1">' +
        ...
        '<div class="pedido-card-qty">' +
          '<span class="pedido-card-qty-main">' + qty + ' <span>' + esc(qtyShort) + '</span></span>' +
    ...
    function renderPedidoCard({ item, qty, img, titulo, m2unit, m2tot, pesoUnit, pesoTot, cxTot, cxpl, sub }, opts) {
      ...
      if (QTY_UNIT === 'palete') {
        ...
        return '<article class="pedido-card' + (forPdf ? ' pedido-card-pdf' : '') + '">' + row1 + spec + '</article>';
      }
    ...
    function renderPedido() {
      ...
      for (const { item, qty } of rows) {
        ...
        const thumbCell = img
          ? '<img src="' + esc(img) + '" alt="" width="48" height="48" style="object-fit:cover" />'
          : '—';
        if (QTY_UNIT === 'palete') {
          const embParts = [];
          ...
          bodyRows.push('<tr>' +
            ...
            '<td class="pedido-col-num pedido-col-preco ' + (descontoPct ? 'has-desc' : '') + '">' + fmtPrecoHtml(item.preco_m2) + '</td>' +
    ...
    function buildPedidoPrintHtml(thumbs) {
      ...
      for (const { item, qty } of rows) {
        ...
        const titulo = item.formigres_titulo || item.descricao;
        cards.push(renderPedidoCard({ item, qty, img, titulo, m2unit, m2tot, pesoUnit, pesoTot, cxTot, cxpl: emb.cxpl, sub }, { pdf: true }));
      }
    ...
    function printPedidoPrintCss(pageWmm, pageHmm) {
      ...
      return pageRule +
        ...
        '.print-resumo { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px 12px; margin-bottom: 10px; }' +
        ...
        '.pedido-card-metrics { margin-top: 10px; }' +
        '.pedido-card-hero { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 6px 12px; }' +
        '.pedido-card-hero-label { display: block; font-size: .58rem; text-transform: uppercase; letter-spacing: .05em; color: #767676; margin-bottom: 2px; }' +
        '.pedido-card-hero-val { display: block; font-size: .86rem; font-weight: 600; color: #2f2f2f; font-variant-numeric: tabular-nums; }' +
        '.pedido-card-hero-item.is-primary .pedido-card-hero-val { font-size: .94rem; font-weight: 700; color: #b01219; }' +
        '.pedido-card-detail { margin-top: 6px; font-size: .66rem; color: #767676; line-height: 1.35; }' +
