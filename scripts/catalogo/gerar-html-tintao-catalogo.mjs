...
function buildHtml({ classif, itens, antLogoDataUri = '', brandLogoDataUri = '', pdfThumbs = {}, pdfFontCss = '', cfg = CFG }) {
  ...
  const pedidoTableHead = isFormigresSkin
    ? '<th>Foto</th><th>Modelo</th><th>Formato</th><th>Paletes</th><th>m² total</th><th>Caixas</th><th>Peso</th><th>Preço/m²</th><th>Subtotal</th>'
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .pedido-card-hero-item.is-primary .pedido-card-hero-val {
      font-size: .98rem;
      font-weight: 700;
      color: var(--text-strong);
    }
...
<body class="is-loading">
  ...
  <script>
    ...
    function renderPedidoCard({ item, qty, img, titulo, m2unit, m2tot, pesoUnit, pesoTot, cxTot, cxpl, sub }, opts) {
      ...
      if (QTY_UNIT === 'palete') {
        const detailParts = [];
        if (cxTot) detailParts.push(fmtDecimal(cxTot, 0) + ' cx');
        if (cxpl) detailParts.push(cxpl + ' cx/pl');
        if (m2unit) detailParts.push(fmtDecimal(m2unit) + ' m²/pl');
        const preco = precoEfetivo(item.preco_m2);
        if (preco) detailParts.push(fmtMoney(preco) + '/m²');
        const detailHtml = detailParts.length
          ? '<p class="pedido-card-detail">' + esc(detailParts.join(' · ')) + '</p>'
          : '';
        return '<article class="pedido-card' + (forPdf ? ' pedido-card-pdf' : '') + '">' +
          '<div class="pedido-card-head">' +
            ...
            '</div>' +
          '</div>' +
          '<div class="pedido-card-metrics">' +
            '<div class="pedido-card-hero">' +
              '<div class="pedido-card-hero-item">' +
                '<span class="pedido-card-hero-label">' + esc(QTY_LABEL) + '</span>' +
                '<span class="pedido-card-hero-val">' + qty + '</span>' +
              '</div>' +
              '<div class="pedido-card-hero-item">' +
                '<span class="pedido-card-hero-label">m² total</span>' +
                '<span class="pedido-card-hero-val">' + esc(m2tot ? fmtDecimal(m2tot) : '—') + '</span>' +
              '</div>' +
              '<div class="pedido-card-hero-item is-primary">' +
                '<span class="pedido-card-hero-label">Peso</span>' +
                '<span class="pedido-card-hero-val">' + esc(pesoTot ? fmtKg(pesoTot) : '—') + '</span>' +
              '</div>' +
            '</div>' +
            detailHtml +
          '</div>' +
      ...
      return '<article class="pedido-card' + (forPdf ? ' pedido-card-pdf' : '') + '">' +
        ...
        '<div class="pedido-card-grid">' +
          '<div class="pedido-card-kv"><span class="pedido-card-kv-label">' + esc(QTY_LABEL) + '</span><span class="pedido-card-kv-val">' + qty + '</span></div>' +
          '<div class="pedido-card-kv"><span class="pedido-card-kv-label">m²/cx</span><span class="pedido-card-kv-val">' + esc(m2unit ? fmtDecimal(m2unit) : '—') + '</span></div>' +
    ...
    function renderPedido() {
      ...
      for (const { item, qty } of rows) {
        ...
        const paleteExtraCols = QTY_UNIT === 'palete'
          ? '<td>' + (cxTot ? fmtDecimal(cxTot, 0) : '—') + '</td>' +
            '<td class="pedido-col-peso">' + (pesoTot ? fmtKg(pesoTot) : '—') + '</td>'
          : '';
        const tintaoMidCols = QTY_UNIT === 'palete'
          ? ''
          : '<td>' + (m2unit ? fmtDecimal(m2unit) : '—') + '</td>';
        const embalagemHint = QTY_UNIT === 'palete' && emb.cxpl
          ? '<br><small class="pedido-row-hint">' + esc(emb.cxpl + ' cx/pl · ' + (m2unit ? fmtDecimal(m2unit) + ' m²/pl' : '—')) + '</small>'
          : '';
        bodyRows.push('<tr>' +
          '<td>' + (img ? '<img src="' + esc(img) + '" alt="" width="48" height="48" style="object-fit:cover;border-radius:6px" />' : '—') + '</td>' +
          '<td><strong>' + esc(titulo) + '</strong><br><small>' + esc(item.codigo_tintao) + '</small>' + embalagemHint + '</td>' +
    ...
    function printPedidoPrintCss(pageWmm, pageHmm) {
      ...
      return pageRule +
        ...
        '.pedido-card-metrics { margin-top: 10px; }' +
        '.pedido-card-hero { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px 10px; }' +
        '.pedido-card-hero-item { min-width: 0; padding: 7px 8px; border: 1px solid #e8e8e8; background: #f7f7f7; }' +
        '.pedido-card-hero-item.is-primary { background: #fff5f5; border-color: #f0c8ca; }' +
        ...
        '.pedido-card-hero-item.is-primary .pedido-card-hero-val { font-size: .94rem; font-weight: 700; }' +
