...
const PREGO_MEDIDA_RE = /^\d+(?:\.\d+\/\d+)?\s+X\s+\d+/i;

function isPregoMedida(value = '') {
  const s = String(value ?? '').trim();
  if (!s) return false;
  return PREGO_MEDIDA_RE.test(s);
}

function isPregoCatalogRow(ctx = {}) {
  const linha = norm(linhaBase(ctx.linha_origem || ctx.linha || ''));
  const core = norm(ctx.core_origem || ctx.core || '');
...
export function normalizeComponentes(comp1, comp2, comp3, ctx = {}) {
  ...
  // Prego — produto compra = PREGO; medida (1 X 15, 1.1/2 X 14…) = comp2.
  if (isPregoCatalogRow(ctx)) {
    const medida = isPregoMedida(c2) ? c2 : isPregoMedida(c1) ? c1 : '';
